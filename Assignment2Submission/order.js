function getTotal(){
    ...
    var q = document.getElementById("productquantity");
    var total = parseInt(price) * parseInt(q);
    document.getElementById("total").innerHTML = "Calculating total...";
}
...
function showCityHint(str) {
    if (str.length == 0) {
        document.getElementById("textHintcity").innerHTML = "";
        return;
    } else {
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {
            if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
                document.getElementById("textHintcity").innerHTML = xmlhttp.responseText;
            }
        };
        xmlhttp.open("GET", "gethint.php?city=" + str + "&state=&zip=", true);
        xmlhttp.send();
    }
}
function showStateHint(str) {
    if (str.length == 0) {
        document.getElementById("textHintstate").innerHTML = "";
        return;
    } else {
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {
            if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
                document.getElementById("textHintstate").innerHTML = xmlhttp.responseText;
            }
        };
        xmlhttp.open("GET", "gethint.php?city=&state=" + str + "&zip=", true);
        xmlhttp.send();
    }
}
function showZipHint(str) {
    if (str.length == 0) {
        document.getElementById("textHintzip").innerHTML = "";
        return;
    } else {
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {
            if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
                document.getElementById("textHintzip").innerHTML = xmlhttp.responseText;
            }
        };
        xmlhttp.open("GET", "gethint.php?city=&state=&zip=" + str, true);
        xmlhttp.send();
    }
}
function getInfo() {
